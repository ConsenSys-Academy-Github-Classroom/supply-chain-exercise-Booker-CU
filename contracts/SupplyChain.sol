...
contract SupplyChain {
  ...
  modifier checkValue(uint _sku) {
    ...
    _;
     ...
     items[_sku].buyer.transfer(amountToRefund); //items[_sku].buyer
  }
  ...
  // an Item has been added?
   ...
   modifier forSale (uint  _sku) {
      require((items[_sku].seller != address(0)) && (items[_sku].state == State.ForSale), "Error item not for sale");
     _;
   }
